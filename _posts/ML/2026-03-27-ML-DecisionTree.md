...
---
  ...
  7. 면접 대비: 핵심 체크리스트 (Interview Points)

   * Q: 결정 트리의 Inductive Bias는 무엇인가요?
       * A: "더 짧은 트리가 더 긴 트리보다 선호된다(Occam's
         Razor)"와 "정보 이득이 높은 특징이 루트에 가까워야
         한다"는 것입니다.
   * Q: 왜 결정 트리는 스케일링이 필요 없나요?
       * A: 트리는 피처의 절대적인 값이 아니라 '값의 순서'를
         기준으로 분할하기 때문입니다. 선형 모델처럼 가중치를
         곱하는 과정이 없습니다.
   * Q: 지니 계수와 엔트로피 중 무엇을 써야 하나요?
       * A: 실질적인 정확도 차이는 거의 없습니다. 다만 계산 비용
         측면에서 지니 계수가 유리하므로 기본값으로 둡니다.
