(function () {
  ...
  $(function () {
    $(document).click(function (e) {
      ...
      if (
        _opened === true &&
        $(e.target).parents(".top-nav-buttons").length == 0
      ) {
...
(function () {
  ...
  $(function () {
    ...
    var tooltipHideTmr;
    $('[data-toggle="tooltip"]:not([data-tooltip-no-hide])').on(
      "inserted.bs.tooltip",
      function () {
...
(function () {
  "use strict";
  var globals = { showToolTip: showToolTip };
...
(function () {
  ...
  function themeToSwitch(checkBox) {
    ...
    const colorSetting =
      ...
      document.body.getAttribute(colorScheme.modeAttr);
    checkBox.checked =
      colorSetting == null || colorSetting == colorScheme.mode.light
        ? false
        : true;
    checkBox.addEventListener("change", switchToTheme);
  }
  function switchToTheme(e) {
    const colorSetting = e.target.checked
  ...
  function initColorSchemeSwitch() {
    ...
    if (switches == null || switches == "undefined" || switches?.length == 0)
      return;
...
(function () {
  ...
  function ToggleShowHide() {
    let topNavOn =
      parseInt(
        window.getComputedStyle(rootElement).getPropertyValue(sideNavWidthVar)
      ) == 0
        ? true
        : false;
    if (topNavOn == true) {
      let sideNavOn =
        parseInt(
          window.getComputedStyle(sideNavElement).getPropertyValue("left")
        ) == 0
          ? true
          : false;
      let sideNavWidth = parseInt(
        window
          .getComputedStyle(rootElement)
          .getPropertyValue(sideNavWidthDefVar)
      );
      let sideNavHeight = parseInt(
        window.getComputedStyle(rootElement).getPropertyValue(sideNavHeightVar)
      );
      if (sideNavOn == false) {
        ...
        if (closeButton) {
          ...
          let middleOfButton =
            parseInt(
              window.getComputedStyle(closeButton).getPropertyValue("height")
            ) / 2;
          closeButton.style.top =
            sideNavHr.getBoundingClientRect().y - middleOfButton + "px";
        }
  ...
  $(function () {
    ...
    $(window).scroll(function () {
      if ($(this).scrollTop() > 10) {
        if (status == stat.off) {
          status = stat.on;
          button.css({ display: "block", opacity: "0.4" });
        }
      } else {
        if (status == stat.on) {
          status = stat.off;
...
(function () {
  ...
  function setMsgHtml(id, msgHtml) {
    ...
    let oldMsgElement = msgBoxElement.getElementsByClassName(
      globals.msgBoxMsgClass
    )[0];
  ...
  function setCloseCallBack(id, callBack) {
    let msgBoxElement = getMsgBoxElement(id);
    let closeButton = msgBoxElement.getElementsByClassName(
      globals.msgBoxCloseClass
    )[0];
...
(function () {
  ...
  $(function () {
    let msgHtml = null;
    if (
      storageChk.isLocalStorageAvailable() &&
      !localStorage.getItem(globals.storageKey)
    ) {
      ...
      for (let browserLng of navigator.languages) {
        for (let supportedLng of globals.supportedLngList) {
          if (browserLng.startsWith(supportedLng[0])) {
            if (!exists(userLngMatchList, supportedLng[0]))
              userLngMatchList.push(supportedLng);
          }
        }
      }
          ...
          for (let item of globals.existLng) {
            if (matchLng[0] == item[0]) {
              msgHtml +=
                '&nbsp;<a href="' +
                item[2] +
                '" onclick="LangOfferMsgBox.saveAndClose();">' +
                matchLng[3] +
                "</a>";
              foundMatch = true;
            }
...
(function () {
  ...
  function showConsentBar() {
    ...
    let offset = parseInt(
      window.getComputedStyle(rootElement).getPropertyValue(footerHeight)
    );
  ...
  function consentSettingDone(button) {
    switch (button) {
      ...
      case "deny":
        ...
        break;
      default:
        logger("consentSettingDone undefined parameter");
    }
    hideSettings();
  }
  function consentBarDone(button) {
    switch (button) {
      ...
      case "deny":
        ...
        break;
      default:
        logger("consentBarDone undefined parameter");
    }
    hideConsentBar();
  }
  */
  ...
  function setConsents() {
    ...
    if (globals.hideConsentBarWithSaveButton)
      localStorage.setItem(storageKey, true);
  }
...
(function () {
  ...
  function mouseEventInit(panel, header) {
    ...
    function mouseMoveEvent(e) {
      ...
      if (
        panel.offsetHeight >
        window.innerHeight - e.clientY + currentY + headerOffset
      ) {
  ...
  $(function () {
    ...
    if (tocContainer.length > 0) {
      function InitToc() {
        ...
        Toc.init({ $nav: tocOutputTarget, $scope: tocInputSource });
        $("body").scrollspy({ target: tocNavSelector });
        let tocViewId = $("#toc-view-top");
        if (tocViewId.length > 0) {
          let tocViewIdBottom =
            tocViewId.offset().top -
            $(window).scrollTop() +
            tocViewId.outerHeight();
...
(function () {
  ...
  var globals = { setProperties: setProperties, buttonClick: buttonClick };
  const properties = {};
  ...
  function setProperties(_properties) {
    for (var key in properties) {
      if (_properties.hasOwnProperty(key)) {
        ...
        logger(
          "Paginator Page Numbers, property key " +
            key +
            " doesn't have default value and not set in setProperties"
        );
        continue;
      }
  ...
  function getPaginatorContainer(clean = false) {
    ...
    if (resultList.length == 1) {
      ...
      logger(
        properties.paginatorListContainerName + " > resultList obj not found."
      );
      return null;
    }
  }
  ...
  function setPage(resultList) {
    if (activeNo <= 0) {
      activeNo = pageList.indexOf(window.location.pathname) + 1;
      logger("activeNo1 <= 0");
    }
    if (activeNo <= 0) {
      logger("activeNo2 <= 0");
    ...
    resultList.append(
      addButton(properties.firstButtonName, pageList[0], activeNo == 1)
    );
    let pageNo = activeNo - 1;
    pageNo = pageNo <= 1 ? 1 : pageNo;
    resultList.append(
      addButton(properties.prevButtonName, pageList[pageNo - 1], activeNo == 1)
    );
...
(function () {
  ...
  properties.resultNotFoundMsgFormat =
    "We couldn't find anything associated with '{property}' for '{value}' here.";
  ...
  function resultMsgFormatter(
    ...
    if (isEmpty(properties.resultKeyValList) == false) {
      ...
      if (firstCharUpperCase == true)
        property_obj.property = firstChrUppercase(property);
    }
    if (firstCharUpperCase == true)
      property_obj.value = firstChrUppercase(value);
  ...
  function setQueryResult(resultList, posts, page_no) {
    if (page_no == null) {
      ...
      let postCountToShow =
        ($(document).height() - top_margin) / tmp_thumb_height;
  ...
  function setQueryResultNotFoundMsg(resultHeader, property, value) {
    resultHeader.append(
      `<${properties.resultHeaderTagName}>${properties.resultNotFoundTitleText}</${properties.resultHeaderTagName}>`
    );
    let txt = resultMsgFormatter(
      property,
      value,
      properties.resultNotFoundMsgFormat
    );
    resultHeader.append(
      `<${properties.resultMsgTagName}>${txt}</${properties.resultMsgTagName}>`
    );
  }
  function setQueryResultFoundMsg(
    resultHeader,
    property,
    value,
    resultFoundTitleFormat
  ) {
    if (isEmpty(resultFoundTitleFormat) == false) {
      ...
      resultHeader.append(
        `<${properties.resultHeaderTagName}>${txt}</${properties.resultHeaderTagName}>`
      );
    }
  }
  function showAll() {
  ...
  function runQuery(
    property,
    value,
    resultFoundTitleFormat = properties.resultFoundTitleFormat,
    mode = properties.resultQueryDisplayMode
  ) {
    ...
    if (posts.length == 0) {
      ...
      if (mode == outMode.paginator) {
        ...
        if ($(properties.hideElementWhenResultShown).length == 1)
          $(properties.hideElementWhenResultShown).show();
      } else if (mode == outMode.scroll_to_load) {
        arryList = posts;
        logger("scrollToLoadResults turn");
        arry = arryList;
        if ($(properties.hideElementWhenResultShown).length == 1)
          $(properties.hideElementWhenResultShown).hide();
      }
      setQueryResultFoundMsg(
        resultHeader,
        property,
        value,
        resultFoundTitleFormat
  ...
  $(function () {
    ...
    function scrollToLoad() {
      if (
        $(window).scrollTop() >=
        $(document).height() - $(window).height() - tmp_thumb_height
      ) {
        if (arryList.length > 0) {
          let resultList = getContainer(properties.resultListName);
          if (resultList == null) return;
          resultList.append(
            resultFormatter(arryList.pop(), properties.queryResultFormat)
          );
        }
      }
    }
...
(function () {
  ...
  $(function () {
    ...
    if ($(upsideDownMain).length > 0) {
      ...
      const tabItem = $(
        upsideDownMain + ">" + tabItemMain + " [data-toggle='tab']"
      );
      const closeButton = $(
        upsideDownMain + ">" + tabItemMain + " #close_tabs"
      );
      const tabContentMain = ".tab-content";
      const tabContent = $(upsideDownMain + ">" + tabContentMain);
      const heights = $(upsideDownMain + ">" + tabContentMain + ">.tab-pane")
        .map(function () {
      ...
      const CssBorderSize =
        parseInt(tabContent.css("border-top-width")) +
        parseInt(tabContent.css("border-bottom-width"));
      var inProgress = false;
      $(document).click(function (e) {
        if (tabContent.height() == 0) return;
        if (
          $(e.target).parents(upsideDownMain).length == 0 ||
          $(e.target).hasClass(tabItemMain.slice(1)) ||
          (clicktabItemsToClose &&
            $(e.target).parents("li").find("a").attr("data-query-link") !==
              undefined)
        ) {
      ...
      var setTabContentHeight = function (_height) {
        ...
        tabContent.animate({ height: _height }, 500, function () {
      ...
      tabItem.click(function () {
        ...
        if (setToMax && elementHeight < elementMaxHeight)
