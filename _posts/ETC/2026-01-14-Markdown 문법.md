...
sidebar:
  nav: 'counts'
math: true           # 수식 설정
permalink: /posts/markdown expression/
---
...
예제:
$$
X \in \mathbb{R}, \quad \mathbb{E}[X] = \mu
$$
...
수식 안에서 **기울어지면 안 되는 텍스트**나  
분산, 로그 등 **연산자 이름**을 표현할 때 사용  

| 표현 | 의미 | Code |
|:-:|---|:--|
| $\mathrm{Var}(X)$ | 분산 | `\mathrm{Var}(X)` |
| $\mathrm{log}(x)$ | 로그 | `\mathrm{log}(x)` |
| $\mathrm{d}x$ | 미분 기호 | `\mathrm{d}x` |
...
## 🔟 괄호 (Brackets & Delimiters)

수식에서 괄호는 **연산 우선순위**, **구조 표현**, **범위 명시**를 위해 사용  
LaTeX에서는 괄호 크기를 자동 조절하거나, 특정 의미의 괄호를 구분해 사용함
...
| $[\ ]$ | 구간, 벡터 요소 | `[ a, b ]` |
| ${ x }$ | 집합 | `{ x }` |
| $\langle\ \rangle$ | 내적, 평균 | `\langle x, y \rangle` |
| $\vert\ \vert$ | 절댓값 | `\vert x \vert` |
| $\Vert\ \Vert$ | 노름 | `\Vert x \Vert` |

예제:
$$
x \in [0, 1], \quad \| \mathbf{x} \|_2
$$

---

### 🔹 자동 크기 조절 괄호 (`\left`, `\right`)

복잡한 수식에서는 **괄호 크기를 자동으로 조절**

| 표현 | Code |
|---|---|
| $\left( \frac{a}{b} \right)$ | `\left( \frac{a}{b} \right)` |
| $\left[ \sum_i x_i \right]$ | `\left[ \sum_i x_i \right]` |
| $\left\{ x \mid x > 0 \right\}$ | `\left{ x \mid x > 0 \right}` |

예제:
$$
\left( \frac{x^2}{y} \right) + \left[ \sum_{i=1}^n x_i \right]
$$

📌 **주의**
- `\left` 와 `\right` 는 항상 **쌍으로 사용**
- 한쪽만 쓰고 싶을 땐 `.` 사용
