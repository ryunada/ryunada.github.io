...
기존 방식 (One-hot Encoding):
  ...
  → 파랑: [0, 1, 0]
  → 초록: [0, 0, 1]
  문제: 고유값이 많으면 차원 폭발, 타깃 누수 위험

CatBoost 방식 (Ordered Target Statistics):
  색상 = 빨강  →  해당 범주의 타깃 평균값(순서 보장)
  색상 = 파랑  →  해당 범주의 타깃 평균값(순서 보장)
  → 차원 증가 없음, 타깃 누수 없음
```
...
```
     ┌─────────────────────────────────────┐
     │   순서 무작위화 (Ordered Boosting)   │  ← 타깃 누수 방지
     └─────────────────┬───────────────────┘
                       │
                       ▼
     ┌─────────────────────────────────────┐
     │   범주형 특성 → 수치 변환            │  ← Ordered Target Statistics
     └─────────────────┬───────────────────┘
                       │
                       ▼
     ┌─────────────────────────────────────┐
     │   잔차 계산 (Gradient 기반)          │  ← Gradient Boosting과 동일
     └─────────────────┬───────────────────┘
                       │
                       ▼
     ┌─────────────────────────────────────┐
     │   Symmetric Tree 학습               │  ← 좌우 대칭 트리
     └─────────────────┬───────────────────┘
                       │
                  n_estimators번 반복
                       │
